...
import {QnaFormData, QnaFormProps} from "@/types/qna";


const QnaForm = ({ mode }: QnaFormProps) => {
    ...
    const onSubmit: SubmitHandler<QnaFormData> = async (data) => {
        if (isEdit) {
            const fieldsToWatch: (keyof QnaFormData)[] = ['title', 'content'];
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await apiHelper.get<QnaFormData>(`/mypage/qna/${id}`);
                reset({
                    title: res.title,
                    content: res.content
                });
