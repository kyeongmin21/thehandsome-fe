...
export const BRAND_LIST: Record<string, string[]> = {
...
// 메인 메뉴 타입 정의
interface MainMenuItemCategory {
    type: 'category';
    label: string;
    data: typeof CATEGORY_LIST;
}

interface MainMenuItemBrand {
    type: 'brand';
    label: string;
    data: typeof BRAND_LIST;
}

interface MainMenuItemSimple {
    type: 'simple';
    label: string;
    data: null;
}

export type MainMenuItem = MainMenuItemCategory | MainMenuItemBrand | MainMenuItemSimple;

export const MAIN_MENU: MainMenuItem[] = [
    {type: 'category', label: '카테고리', data: CATEGORY_LIST},
    {type: 'brand', label: '브랜드', data: BRAND_LIST},
    {type: 'simple', label: '아울렛', data: null},
    {type: 'simple', label: '셀렉티드', data: null},
    {type: 'simple', label: 'TOP 100', data: null},
];